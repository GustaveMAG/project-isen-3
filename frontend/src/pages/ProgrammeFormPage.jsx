...
export default function ProgrammeFormPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
        await programmesApi.update(id, form);
        toast.success('Projet mis à jour');
        navigate(`/programmes/${id}`);
      } else {
        const { data } = await programmesApi.create(form);
  ...
  return (
    <div className="max-w-xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        <Link to="/programmes" className="text-gray-400 hover:text-gray-700">← Retour</Link>
      ...
      <div className="card">
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            <label className="label">Sujet / description</label>
            <textarea
              className="input resize-none"
              rows={5}
              value={form.description}
              onChange={(e) => setForm({ ...form, description: e.target.value })}
              placeholder="Décrivez le sujet du projet, commun à toutes les équipes…"
            />
          </div>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <Link to={isEdit ? `/programmes/${id}` : '/programmes'} className="btn-secondary">
              Annuler
            </Link>
          </div>
        </form>
